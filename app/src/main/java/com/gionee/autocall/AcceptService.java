...
//    TelephonyManager manager = (TelephonyManager) getSystemService(TELEPHONY_SERVICE);
    public int onStartCommand(Intent intent_acc, int flags, int startId) {
        ...
        writeData("测试次数,来电次数,接听次数,失败次数,成功率");

        mPhoneListener.register(new phoneListener.phoneStateListener() {
            TelephonyManager manager = (TelephonyManager) getSystemService(TELEPHONY_SERVICE);
            @Override
            public void onStateRINGING(String phony) {
                if (phony.equals(PHONE_NUMBER)) {
                    Log.d("KEVIN", "onStateRINGING: "+phony);
...
//});
    ...
    public void onDestroy() {
        ...
        if(EXCUTE_COUNT>0){
            writeData(TEST_LOOPS+","+String.valueOf(EXCUTE_COUNT)+","+String.valueOf(PASS_COUNT)+","+String.valueOf(FAIL_COUNT)+","+String.valueOf(df.format((float) PASS_COUNT / EXCUTE_COUNT)));
        }
        super.onDestroy();
    }
